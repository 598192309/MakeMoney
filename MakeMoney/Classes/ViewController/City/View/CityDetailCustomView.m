...
- (void)configUIWithItem:(CityListItem *)item finishi:(void(^)(void))finishBlock{
    ...
    NSAttributedString *att = [item.text lq_getAttributedStringWithLineSpace:2 kern:0 aliment:NSTextAlignmentLeft];
...
-(UIView *)header{
    if (!_header) {
       ...
       _timeLabel = [UILabel lableWithText:lqStrings(@"") textColor:TitleGrayColor fontSize:AdaptedFontSize(12) lableSize:CGRectZero textAliment:NSTextAlignmentCenter numberofLines:0];
       [contentV addSubview:_timeLabel];
       [_timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
           
           make.left.mas_equalTo(Adaptor_Value(10));           make.top.mas_equalTo(Adaptor_Value(15));
       }];
       
       _detailLabel = [UILabel lableWithText:@"" textColor:TitleWhiteColor fontSize:AdaptedFontSize(15) lableSize:CGRectZero textAliment:NSTextAlignmentCenter numberofLines:0];
       ...
       _contactBtn.titleLabel.font = AdaptedFontSize(15);
