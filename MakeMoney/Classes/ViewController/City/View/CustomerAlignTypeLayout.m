...
@interface CustomerAlignTypeLayout(){
    //在居中对齐的时候需要知道这行所有cell的宽度总和
    CGFloat _sumWidth ;
}
@end

@implementation CustomerAlignTypeLayout

-(instancetype)init{
    self = [super init];
    if (self){
        self.scrollDirection = UICollectionViewScrollDirectionVertical;
        self.minimumLineSpacing = Adaptor_Value(10);
        self.minimumInteritemSpacing = Adaptor_Value(5);
        //        self.sectionInset = UIEdgeInsetsMake(5, 5, 5, 5);
        _betweenOfCell = Adaptor_Value(5);
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (NSUInteger index = 0; index < layoutAttributes.count ; index++) {
        ...
        //加入临时数组
        [layoutAttributesTemp safeAddObject:currentAttr];
